...
if [[ ${PV} == 9999 ]]; then
	inherit git-r3
	EGIT_REPO_URI="https://dawn.wine/dawn-winery/wine-dwproton.git"
	EGIT_BRANCH="bleeding-edge"
else
	# hopefully replaced with an archive link sometime
	inherit git-r3
	EGIT_REPO_URI="https://dawn.wine/dawn-winery/wine-dwproton.git"
	EGIT_BRANCH="dwproton/${WINE_PV}"
	EGIT_CLONE_TYPE="shallow"
	#SRC_URI="https://dawn.wine/dawn-winery/wine-dwproton/archive/refs/tags/proton-wine-${WINE_PV}.tar.gz"
	#S=${WORKDIR}/${PN}-wine-${WINE_PV}
	KEYWORDS="-* ~amd64 ~x86"
fi

DESCRIPTION="Wine-cachyos with a bit of extra spice"
HOMEPAGE="https://dawn.wine/dawn-winery/wine-dwproton"
...
src_prepare() {
	...
	sed -i "s/wine_build[^1]*1/& (DWProton-${WINE_PV})/" configure.ac || die
